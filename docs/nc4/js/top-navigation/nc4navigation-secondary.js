...
var DOMContextSelector = function (dom, options, msg) {
    ...
    function createExtraMenuItemMyProfile(data) {
    ...
    function createExtraMenuItemLanguage(data) {
        var li = dom.createElement('li', ['clear'], [{
            role: 'menuitem'
        }]),
    ...
    function createExtraMenuItemMyLocation(data) {
        var li = dom.createElement('li', ['clear'], [{
            role: 'menuitem'
        }]),
    ...
    function createExtraMenuItemCopyright(data) {
        var li = dom.createElement('li', ['clear'], [{
            role: 'menuitem'
        }]),
        a4 = dom.createElement('p', null, [{ href: '#' }], 'Visma Recruit Copyright 2021 Visma.');

        li.appendChild(a4);
        return li;
    }

    function createExtraMenuItemLicence(data) {
        var li = dom.createElement('li', ['clear'], [{
            role: 'menuitem'
        }]);
        let a5 = document.createElement('p');
        a5.innerHTML = 'Visma Recruit is made possible by Visma and other <a href="#">open source and third party licenses</a>.';

        li.appendChild(a5);
        return li;
    }

    function createExtraMenuItemTerms(data) {
        var li = dom.createElement('li', ['clear'], [{
            role: 'menuitem'
        }]),
        a6 = dom.createElement('a', null, [{ href: '#' }], 'Terms of Service');
           
        li.appendChild(a6);
        return li;
    }

    function createExtraMenuItemDivider(data) {
    ...
    function createExtraMenuItemLightDark(data) {
        var li = dom.createElement('li', ['clear'], [{
            role: 'menuitem'
        }]);
        ...
        label.className = 'switch switch-label light-dark-toggle';
        label.innerHTML = '<b class="light">Light mode</b><input id="option" type="checkbox" name="option"><span class="togglemark"><b class="details"></b></span><b class="dark">Dark mode</b>';
    ...
    function createFooter() {
        var li = dom.createElement('li', ['context-footer', 'company-selection-footer', 'clear'], [{
            ...
            a = dom.createElement('a', ['log-out-link', 'vismaicon', 'vismaicon-sm', 'vismaicon-logout'], [{ id: constants.id.logoutButton }], msg.logout);
        li.appendChild(a);
        return li;
    }
    ...
    function createDropdownMenu(data) {
        ...
        ul.appendChild(createExtraMenuItemMyProfile());
        ul.appendChild(createExtraMenuItemLanguage());
        ul.appendChild(createExtraMenuItemMyLocation());
        ul.appendChild(createExtraMenuItemDivider()); // dotted divder
        ul.appendChild(createExtraMenuItemCopyright()); // copyright
        ul.appendChild(createExtraMenuItemLicence()); // copyright
        ul.appendChild(createExtraMenuItemTerms()); // copyright
        ul.appendChild(createExtraMenuItemDivider()); // dotted divder
        ul.appendChild(createExtraMenuItemLightDark()); // light/dark mode shitcher
    ...
    function init() {
        ...
        function createComponent(msg, options, data, comp) {
            var root = document.getElementById(constants.id.component),
                header = dom.createElement('header', ['navbar', 'navbar-secondary'], [{
